...
object mars {
  ...
  def calcAccelWithAngle(angleDeg : Int, push : Int) = {

      val p = (point.angulNormDegree(90 - angleDeg) * push)
      val pg = p ++ GRAV
      pg
  ...
  class vehicule(coord: point, val velocity: point) {
    ...
    def gotoV(c: point): (Double, Double) = {
      ...
      val limitVel=30
      val nbStepToSpeed0 = Math.max((velocity.y.abs-limitVel),0) / stableVVel
      val distToSpeed0 = nbStepToSpeed0 * velocity.y.abs - stableVVel * nbStepToSpeed0

      val dirVel = sign(velocity.y)

Console.err.println(""+(dy.abs - 200)+" "+velocity.y)

        if (distToSpeed0 < dy.abs - 200 || velocity.y > 1) {
          (0, 0)
        } else if (distToSpeed0 > dy.abs-10 || velocity.y < -limitVel) {
          (0, 4)
        } else {
          (0, 3)
        }
    ...
    def gotoHFastest(c: point): (Double, Double) = {
      val dx = c.x - coord.x
      
      val angl=60
      val push =4
      val xPush=calcAccelWithAngle(angl,push)

      //val nbStepToSpeed0 = velocity.x.abs / xPush.x.abs

      //val distToSpeed0 = nbStepToSpeed0 * velocity.x.abs - xPush.x.abs * nbStepToSpeed0
