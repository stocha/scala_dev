 ...
 * @author Jahan
 */
class tb004 extends agentAbstract {
  ...
  def doPlan(ref: GameState4P) = {
    ...
    val tmpF0 = for (d <- 0 until 4; s <- List(4,8,16)) yield {
      (new Tuple2(d, s))
    }
    //val bmToTry = ((genBmSquare(tmpF0, bv).sortBy { y => -(y.countBitset) }).filter { x => x.countBitset > 0 }).map { x => val = x.sp }.toList ::: ref.tr.void.split
    val bmToTry=ref.tr.void.split
    
    
    val maxDest = if(bmToTry.nonEmpty) bmToTry.maxBy { x => bv.eval_withTaker(x) } else BMap.full

    val taker = new bv_taker(maxDest, 0xFF839298L)
    //currPlan=taker

    System.err.println("Planning to "+maxDest);
