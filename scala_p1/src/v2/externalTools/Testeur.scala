...
def almEmptym : List[String]=List(
...
def pathTestTo=List(
  ...
  def testPathOpti(){
   ...
   val reward=BMap.fromString(pathTest)
   val f= BMap.fromString(pathTestFrom)
   val t= BMap.fromString(pathTestTo)
   
   System.err.println("reward"+reward);
    System.err.println("from"+f);
     System.err.println("to"+t);
   
   System.err.println(bitStack.sumFromTo(f,t,reward))
