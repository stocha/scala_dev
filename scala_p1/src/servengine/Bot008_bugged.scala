 ...
 * @author Jahan
 */
class Bot008_DefShortcut extends servBot {
  var nbPlay = -1;
  var idP = -1;
  ...
  def stratBase ={
      ...
      //  Console.err.println("maxEmpty\n"+maxEmpty);
       ...
       // Console.err.println("captIfCross\n"+captIfCross);
        ...
        val cross=BitMap.zero.paintCrossAt(coord.x, coord.y) & void // important : uniquement les cases vides vieux !!
        //Console.err.println("cross\n"+cross);
        val captIfCross=extractMax(BitMap.enclosed((terri | cross), (void & (~(terri | cross))) ) & firstZone , 2)      
        //Console.err.println("captIfCross\n"+captIfCross);
        
        val toCapt= (captIfCross.scramble & (void^captIfCross)) & (~terri)
        //Console.err.println("toCapt\n"+captIfCross);
        
        goTarget(coord,List(toCapt ,clothestTo,bordFirstZone,maxEmpty), void)
